...
/*
 ...
 7. Properties for component
   7a. Use directives for properties
     7aa. Directives for properties must be created when components directive first time used.
     7ab. each property directive must have restriction to be used only with component directive.
     7ac. Component directive may be A or E. Only E/AE creates property directives.
   7b. Use scope param of component directive scope: {param: "=?"}
     7ba. Check if its possible to setup facade object instead of scope as target scope for properties, like scope { "facade.property": "=?" }.
 */
/*
{
  name: "name of directive",
 propertyPrefix: "prefix for property attributes, if defined, attreibutes will be prefix-propertyName",
  link:{
    pre: function (){
      // component handler overwrite
      1. controller should be created, check if this is first time
      2. if yes, ask for facade
      3. go through its properties and register them to the directive
    }
  }
  scope:{

  },
  properties: [ add listed properties to scope if they are not defined there with =? keyword ]
}
*/
/**
 *
 * @param {string|{name:string, properties:array,templateUrl:string,template:string}} directiveOptions
 * @param {Function} componentDefinition
 * @param {Function} controllerDefinition
 * @static
 */
